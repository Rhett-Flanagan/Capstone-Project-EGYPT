...
class TestSetupMethods(unittest.TestCase):
    ...
    def testConstructor(self):
        ...
        self.assertEqual(sim.startingHouseholds, 2)
        self.assertEqual(sim.startingHouseholdSize, 2)
        self.assertEqual(sim.startingGrain, 5000)
        self.assertEqual(sim.minAmbition, 0.5)
        self.assertEqual(sim.minCompetency, 0.1)
        self.assertEqual(sim.generationalVariation, 0.7)
        ...
        self.assertEqual(sim.fissionChance, 0.5)
    ...
    def testGridSetup(self):
        ...
        nSettlement = 0
        territory = True
        grid = sim.grid.get_neighbors((2, 2), True, True, 2)
...
class TestDataCollectorMethods(unittest.TestCase):
    ...
    def testGini(self):
        ...
        self.assertAlmostEqual(gini(sim), 0) # Almost Equal for float comparison, should be zero at start for perfect equality at start
        ...
        for h in sim.schedule.get_breed(Household):
            ...
            if wealth:
                ...
                h.grain = 0
        self.assertAlmostEqual(gini(sim), 0.75) # Almost Equal for float comparison, will be 1 for large numbers of households, 0.75 for the small number here
        ...
        self.assertAlmostEqual(gini(sim), 0.25) # Almost Equal for float comparison

        # Force 0 Households, divide by 0 error
        sim = EgyptSim(height=5, width=5, timeSpan=10, startingSettlements=0, startingHouseholds=0,
    ...
    def testMinSetPop(self):
        ...
        # Equal populations
        sim = EgyptSim(height=5, width=5, timeSpan=10, startingSettlements=4, startingHouseholds=2,
                   startingHouseholdSize=2, startingGrain=5000, minAmbition=0.5, minCompetency=0.1,
                   generationalVariation=0.7, knowledgeRadius=2, distanceCost=10, fallowLimit=1,
                   popGrowthRate=0.2, fission=False, fissionChance=0.5, rental=False, rentalRate=0.1)
    ...
    def testLowerThirdGrainHoldings(self):
        ...
        sim = EgyptSim(height=5, width=5, timeSpan=10, startingSettlements=3, startingHouseholds=2,
                   startingHouseholdSize=2, startingGrain=5000, minAmbition=0.5, minCompetency=0.1,
                   generationalVariation=0.7, knowledgeRadius=2, distanceCost=10, fallowLimit=1,
                   popGrowthRate=0.2, fission=False, fissionChance=0.5, rental=False, rentalRate=0.1)
        i = 1000
